...
namespace M450_FileSorter.Tests.Rules
{
    public class RegexRuleTests
    {
        ...
        public void Matches_WithOptionalBasePattern_RequiresBothPatternsToMatch()
        {
            ...
            { "base_pattern", @"\.pdf$" } // Simuliertes optionales Pattern (normalerweise 'pattern' in BaseRule)
            // In der echten BaseRule wird 'pattern' für das optionale Pattern verwendet.
            // Hier simulieren wir es, da RegexRule 'pattern' für sein Haupt-Regex verwendet.
            // Für einen echten Test müsste man die BaseRule-Logik mocken oder komplexer aufbauen.
            // Einfacher Test: Wir gehen davon aus, dass BaseRule.MatchesPattern korrekt funktioniert.
            // Wir testen hier primär die Logik von RegexRule selbst.
        });

            // Wir testen hier nur das Haupt-Regex, da das optionale Pattern in BaseRule liegt.
            // Ein Integrationstest oder ein Test mit angepasster BaseRule wäre nötig, um beides zu prüfen.
            Assert.True(rule.Matches(_invoiceFile)); // Haupt-Regex passt
        }
