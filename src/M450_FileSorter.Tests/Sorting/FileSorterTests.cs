...
{
    public class FileSorterTests : IDisposable // IDisposable für Cleanup
    {
        ...
        public FileSorterTests()
        {
            ...
            // Standard-Setup für Mocks (kann in Tests überschrieben werden)
        ...
        // Cleanup nach jedem Test (optional, aber gut für Dateisystem-Mocks)
        public void Dispose()
        {
            // Hier könnten temporäre Dateien/Verzeichnisse gelöscht werden, falls erstellt
        }
        ...
        public void SortFiles_SimulationMode_DoesNotCreateOrMove()
        {
            ...
            _mockFileSystem.Verify(fs => fs.CreateDirectory(It.IsAny<string>()), Times.Never);
            _mockFileSystem.Verify(fs => fs.MoveFile(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
            // Optional: Überprüfen, ob die Simulations-Logausgabe erfolgt ist (schwierig ohne Logger-Mock)
        }
        ...
        public void SortFiles_GetFileMetadataReturnsNull_SkipsFile()
        {
            ...
            // Setup: GetFileMetadata gibt null zurück (z.B. Datei wurde inzwischen gelöscht)
